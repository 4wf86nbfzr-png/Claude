...
export function Kennzahl({ wert, label, farbe = 'grau', href, hinweis }: { wert: ReactNode; label: string; farbe?: Farbe; href?: string; hinweis?: string }) {
  const inhalt = (
    <>
      ...
      <span style={{ fontSize: 12, color: 'var(--text-sekundaer)' }}>{label}</span>
      {hinweis && <span style={{ fontSize: 11, color: 'var(--text-gedaempft)' }}>{hinweis}</span>}
    </>
  );
  const stil: React.CSSProperties = {
    display: 'flex', flexDirection: 'column', gap: 2, padding: '14px 16px',
    background: 'var(--flaeche-karte)', border: '1px solid var(--linie)',
    borderRadius: 'var(--radius)', boxShadow: 'var(--schatten)', textDecoration: 'none',
  };
...
export function Balken({ ist, soll }: { ist: number; soll: number }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      <span aria-hidden style={{ width: 54, height: 5, borderRadius: 3, background: 'var(--flaeche-tief)', overflow: 'hidden', flex: 'none' }}>
        <span style={{ display: 'block', width: `${prozent}%`, height: '100%', background: farbe }} />
      </span>
      <span className="zahl" style={{ fontSize: 12, color: 'var(--text-sekundaer)' }}>{ist}/{soll}</span>
    </span>
  );
}
  ...
  return (
    <header style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 18 }}>
      <div style={{ minWidth: 0 }}>
        ...
        {unter && <div style={{ fontSize: 13, color: 'var(--text-sekundaer)', marginTop: 3 }}>{unter}</div>}
      </div>
...
export function Hinweis({ art = 'info', children }: { art?: 'info' | 'warnung' | 'fehler' | 'erfolg'; children: ReactNode }) {
  ...
  return (
    <p role={art === 'fehler' ? 'alert' : undefined}
       style={{ background: farben[0], color: farben[1], border: `1px solid ${farben[1]}33`, borderRadius: 'var(--radius-s)', padding: '9px 12px', fontSize: 13, margin: 0 }}>
      {children}
    </p>
  );
}
...
export function Paar({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
      <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '.04em', textTransform: 'uppercase', color: 'var(--text-gedaempft)' }}>{label}</span>
