 ...
 * fuer den Mitarbeiter sichtbar, und die Disposition wird benachrichtigt –
 * ein hochgeladener Nachweis nuetzt nur, wenn ihn jemand prueft.
 */
export async function eigenesDokumentAktion(_zustand: Ergebnis, formData: FormData): Promise<Ergebnis> {
  return fuehreAus(async () => {
    ...
    await notifyDispo({
      ...
      body: `${dokument.title} (${typ.toLowerCase()}) wurde hochgeladen und wartet auf Pruefung.`,
