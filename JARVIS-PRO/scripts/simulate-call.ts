...
/**
 * Drehbuch fuer das Gehirn.
 *
 * Bewusst ein Drehbuch und kein Modell: der Simulator soll den ABLAUF zeigen,
 * nicht die Formulierkunst eines Modells. Was hier passiert, passiert im
 * Betrieb genauso - nur die Saetze kommen dann vom Modell.
 */
function buildSteps(eventRef: { id: string }): ScriptedStep[] {
  return [
    {
    ...
    },
    {
      ...
      tools: [
        ...
        { name: 'request_approval', args: (last: string | null) => ({ draftId: extractId(last) }) },
      ],
...
function extractId(text: string | null): string {
