...
const SCHEMA = z.object({
  employeeId: z.string().trim().min(1, 'Bitte waehlen Sie einen Mitarbeiter.'),
...
export async function zeitSpeichern(user: SessionUser, id: string | null, formData: FormData) {
  ...
  if (minuten === 0) throw new ValidationError('Start und Ende ergeben keine Arbeitszeit. Bitte pruefen Sie die Eingabe.');
  ...
  if (id) {
    ...
    if (vorher.status === 'ABGERECHNET') {
      throw new ValidationError('Abgerechnete Zeiten koennen nicht mehr geaendert werden.');
    }
    ...
    await audit(user, {
      ...
      summary: `Zeiteintrag geaendert (${unterschied.changed.join(', ')})`,
...
export async function zeitenFreigeben(user: SessionUser, ids: string[]): Promise<number> {
  ...
  await audit(user, {
    ...
    summary: `${ergebnis.count} Zeiteintraege freigegeben`,
    after: { ids },
  });
...
export async function zeitEntfernen(user: SessionUser, id: string, grund: string) {
  const eintrag = await db.timeEntry.findFirst({ where: { id, deletedAt: null } });
  if (!eintrag) throw new NotFoundError('Der Zeiteintrag wurde nicht gefunden.');
  if (eintrag.status === 'ABGERECHNET') throw new ValidationError('Abgerechnete Zeiten koennen nicht entfernt werden.');
