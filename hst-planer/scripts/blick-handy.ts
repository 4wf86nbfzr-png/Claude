/** Bildschirmfoto in Handybreite – prüft SecPlan 28. */
...
async function main() {
  const [email = 'max.mustermann@example.org', ...pfade] = process.argv.slice(2);
  ...
  for (const pfad of pfade) {
    const name = 'handy-' + (pfad.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '') || 'start');
